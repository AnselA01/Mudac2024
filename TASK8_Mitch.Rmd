...
big_data_sf %>%
  ...
  theme_bw()+
  geom_sf(data = ethanol_sf, color = "blue") +
  geom_sf(data = guesses_sf, color = "green")


  roads_sf <- sf::st_read("Mudac2024/Data/mn_roads_shp-2/tl_2019_27_prisecroads.shp")
    wat_sf <- sf::st_read("Mudac2024/Data/shp_water_lakes_rivers/LakesAndRivers.shp")
  
    st_crs(wat_sf)
    
  roads <- roads_sf %>%
    filter(RTTYP %in% c("I", "U")) %>%
    st_transform(crs = st_crs(mn_counties))

  guesses <- data.frame(lat = c(44.69, 44),
                      long = c(-95.5, -93.6)) 
guesses

guesses_sf <- st_as_sf(guesses, coords = c("long", "lat")) %>%
...
big_data_sf %>%
  ...
  geom_sf(aes(fill = AD) ,
              show.legend = F) +
  labs(fill = "AD",
       title = "Aggregate Demand for Fertilizer",
       caption = "Blue points are ethanol plants, Green point are our predictions.") +
  scale_fill_fermenter(palette = "Oranges", direction = 1) +
  theme_bw()+
  geom_sf(data = ethanol_sf, color = "blue", size = 3) +
    geom_sf(data = roads, color = "black")+
  geom_sf(data = guesses_sf, color = "green", size = 3.5) +
...
big_data_sf %>%
  ...
  labs(fill = "AD",
       title = "Aggregate Demand for Fertilizer",
       caption = "Blue points are ethanol plants, Red point are our predictions.") +
  scale_fill_fermenter(palette = "Oranges", direction = 1) +
  theme_bw()+
  geom_sf(data = ethanol_sf, color = "blue", size = 3) +
    geom_sf(data = wat_sf, color = "green", alpha = 0.3)+
  geom_sf(data = guesses_sf, color = "red", size = 3.5) +
  theme(plot.title = element_text(size=22))
